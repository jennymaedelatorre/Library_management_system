...
// Query to fetch all books and whether they are borrowed by the current user
...
";
$book_result = executeQuery($conn, $book_query, [$user_id], "Error fetching books");
?>
...
<body>
    ...
    <div class="content mt-5">
        ...
        <!-- List of Books -->
        <h2 class="mt-4" style="font-size: 1.2rem; font-weight: bold; margin-left: 20px;"><i>Explore Books You Like!</i></h2>
    ...
    if (pg_num_rows($book_result) > 0) {
        while ($book = pg_fetch_assoc($book_result)) {
            ...
            if ($is_borrowed) {
                echo "<button class='disable-btn btn-disabled' disabled>Not Available</button>";
            } elseif (!$is_available) {
                echo "<button class='disable-btn btn-disabled' disabled>Not Available</button>";
            } else {
                echo "<a href='#' 
        ...
        echo "<tr><td colspan='6'>No books available in the library.</td></tr>";
    }
    ?>
</tbody>
